...
export async function mergeScrapedListings(scraped, activeProductIds) {
  ...
  for (const item of scraped) {
    ...
    if (existing) {
      ...
      // Only refresh the fields that actually come from Chairish and can
      // legitimately change there (price, photos, the listing URL). Every
      // curatorial field you set here in the dashboard - category, tags,
      // era, origin, condition, description, featured - is preserved
      // rather than being overwritten by the raw scrape on every sync.
      // Previously this spread the entire scraped item over the existing
      // one, which meant any manual edit you made got silently wiped out
      // the next time you clicked Sync Now.
      merged[idx] = {
        ...existing,
        ...
        // Backfill a category for anything imported before auto-categorizing
        // existed, but never overwrite one you've already set.
        category: existing.category || item.category || categorizeListing({ ...existing, ...item }),
        id: existing.id,
      };
      updated += 1;
    } else {
      const id = await uniqueId(slugify(item.title), merged);
      const category = item.category || categorizeListing(item);
      merged.unshift({ ...item, id, category, createdAt: new Date().toISOString() });
      added += 1;
    }
  }
