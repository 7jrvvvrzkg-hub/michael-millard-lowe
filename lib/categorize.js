...
function scoreCategory(text, keywords) {
  ...
  for (const kw of keywords) {
    // Word-boundary match, not a raw substring check - "chair" as plain
    // substring also matches inside "chairish" (the word "Chairish"
    // itself, which shows up constantly since that's where every listing
    // is scraped from), silently mis-tagging unrelated items as furniture.
    const pattern = new RegExp(`\\b${escapeRegex(kw)}(e?s)?\\b`, "i"); // allow plural (vase/vases, sconce/sconces)
    if (pattern.test(text)) score += kw.includes(" ") ? 2 : 1;
  }
  return score;
}
...
function categorizeListing(listing) {
  ...
  // Nothing scored above zero - don't let category array order silently
  // pick a "winner" out of what is really a tie. Keep whatever category
  // was already set, or fall back to "decor" as the actual catch-all
  // (previously this fell through to whichever category happened to be
  // first in CATEGORIES - furniture - on every real tie).
  if (!bestSlug) return listing.category || "decor";
  return bestSlug;
}
