...
async function scrapeProduct(url) {
  ...
  // extractImageUrl is a best-effort walk through whatever shape Chairish's
  // JSON-LD "image" field happens to be - if that shape ever changes again
  // in some way this doesn't recognize, it could still hand back something
  // that isn't a plain string. Explicitly checking typeof here (rather than
  // just trusting a truthy return value) means that case falls through to
  // the og:image meta tag instead of crashing every single listing with
  // "heroImage.split is not a function" the way a stray non-string value
  // used to. og:image has been reliably a plain string on every real
  // listing checked, so it's a safe unconditional fallback.
  const rawImage = extractImageUrl(jsonLd && jsonLd.image);
  const heroImage =
